...
class FormAction(Action):
    ...
    def from_regex(
        ...
        return {"type": "from_regex", "intent": intent, "not_intent": not_intent, "regex": regex}
            ...
            if self.intent_is_desired(requested_slot_mapping, tracker):
                ...
                if mapping_type == "from_entity":
                    ...
                    if re.match(regex, message) == None:
                        ...
                        continue
                elif mapping_type == "from_regex":
                    message = tracker.latest_message.get("text")
                    regex = requested_slot_mapping["regex"]
                    values = re.findall(regex, message)
                    if len(values) > 0:
                        value = values[0]
                    else:
                        continue
        ...
        for slot in self.required_slots(tracker):
            if self._should_request_slot(tracker, slot):
                ...
                if tracker.get_slot("account_" + slot) == None:
                    ...
                    buttons = []
                    buttons.append({"title": tracker.get_slot("account_" + slot), "payload": tracker.get_slot("account_" + slot)})
